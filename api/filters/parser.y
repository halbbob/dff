...
/* Define terminal basic symbols (tokens) and associate variable in typeParser union*/
%token <token> TCOMMA TLPAREN TRPAREN TDOT TLBRACKET TRBRACKET
/* following token are split for more clarity and by type of operation */
%token <token> TAND TOR
%token <token> TEQ TNEQ TLT TGT TLTE TGTE TIN TCONTAINS

/* still terminal symbols and adds node types (relative to typeParser union)*/
%token <number> TNUMBER
%token <boolean> TTRUE TFALSE
%token <identifier> TSTRING TATTRIBUTES TTIMESTAMP
%token <str> TIDENTIFIER

%type <comp> comp_operators
%type <identifier> ident
%type <expression> expr primary_expression processor
...
input: 
expr { ((parserParam*)data)->expression = $1; }
;
...
comp_operators: TEQ { $$ = Comparison::EQ; }
| TNEQ { $$ = Comparison::NEQ; }
| TGT { $$ = Comparison::GT; }
| TGTE { $$ = Comparison::GTE; }
| TLT { $$ = Comparison::LT; }
