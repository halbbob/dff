  ...
  if (which < this->bs->numfat)
    {
      ...
      attrs["free space"] = new Variant(clustsize * this->freeClustersCount(which));
      attrs["allocated clusters"] = new Variant(this->allocatedClustersCount(which));
      attrs["used space"] = new Variant(clustsize * this->allocatedClustersCount(which));
      if ((badclust = this->badClustersCount(which)) != 0)
	{
	  attrs["bad clusters"] = new Variant(this->badClustersCount(which));
	  attrs["bad clusters space"] = new Variant(clustsize * badclust);
	}
      else
	attrs["bad clusters"] = new Variant(0);
    }
  return attrs;
}
