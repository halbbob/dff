...
{
  std::ostringstream	expectedSignature;
  std::ostringstream	inBufferSignature;
  ...
  while (mftEntrySize && i != 4) {
    inBufferSignature.str("");
    inBufferSignature << (char)*(_readBuffer + mftEntrySize);
    inBufferSignature << (char)*(_readBuffer + mftEntrySize + 1);
    inBufferSignature << (char)*(_readBuffer + mftEntrySize + 2);
    inBufferSignature << (char)*(_readBuffer + mftEntrySize + 3);

    for (i = 0; i < expectedSignature.str().size(); i++) {
      /* Char by char because no trailing \0 at end of signature */
      if (expectedSignature.str()[i] != inBufferSignature.str()[i])
	break;
    }
    if (i != 4)
      mftEntrySize *= 2;
