...
class PieView(QtGui.QAbstractItemView):
    ...
    def edit(self, index, trigger, event):
        return False#if index.column() == 0:
            #return True #return super(PieView, self).edit(index, trigger, event)
        #else:
        #    return False
    ...
    def indexAt(self, point):
        ...
        if wx < self.totalSize:
            ...
            for row in range(self.model().rowCount(self.rootIndex())):
                ...
                if value > 0.0:
                    ...
                    if angle >= startAngle and angle < (startAngle + sliceAngle):
                        return self.model().index(row, 1, self.rootIndex()).toInt()[0]
            ...
            for row in range(self.model().rowCount(self.rootIndex())):
                ...
                index = self.model().index(row, 1, self.rootIndex())
                if self.model().data(index).toInt()[0] > 0.0:
    ...
    def itemRect(self, index):
        ...
        if self.model().data(valueIndex).toInt()[0] > 0.0:

            listItem = 0
            for row in range(index.row()-1, -1, -1):
                if self.model().data(self.model().index(row, 1, self.rootIndex())).toInt()[0] > 0.0:
                    listItem += 1
    ...
    def itemRegion(self, index):
        ...
        for row in range(self.model().rowCount(self.rootIndex())):
            ...
            value = self.model().data(sliceIndex).toInt()[0]
